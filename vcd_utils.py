...
def detect_cliches(f: Features) -> List[Dict[str, str]]:
    ...
    if f.edge_density < 0.04:
        signals.append({
            ...
            "desc": "Very low edge detail can drift into generic minimal icon territory.",
            "common_in": "SaaS / Fashion / Lifestyle",
        })
...
def _dedupe_results(items: List[dict], max_results: int) -> List[dict]:
    out: List[dict] = []
    seen = set()

    for it in items:
        norm = _normalize_result_item(it)

        key = (
            norm["link"].lower(),
            norm["thumbnail"].lower(),
            norm["title"].lower(),
        )
...
def _upload_to_cloudinary_cached(image_png_bytes: bytes) -> str:
    ...
    files = {"file": ("image.png", image_png_bytes, "image/png")}
    ...
    return r.json().get("secure_url", "") or ""


@st.cache_data(show_spinner=False)
def _google_lens_search_cached(image_url: str, max_results: int) -> List[dict]:
